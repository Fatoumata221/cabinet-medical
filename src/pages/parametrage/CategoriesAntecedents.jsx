...
const CategoriesAntecedents = () => {
  const [items, setItems] = useState([]);
  const [newItem, setNewItem] = useState({ 
    nom: undefined, description: undefined, ordre_affichage: undefined, actif: undefined
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setNewItem({ nom: undefined, description: undefined, ordre_affichage: undefined, actif: undefined });
      setShowForm(false);
      fetchItems();
    } catch (error) {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{item.ordre_affichage}</td>
...
export default CategoriesAntecedents;