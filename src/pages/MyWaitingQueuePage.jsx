...
const MyWaitingQueuePage = () => {
  ...
  const fetchWaitingQueue = async (profile = userProfile) => {
    try {
      ...
      console.log('✅ [MyWaitingQueue] File d\'attente récupérée:', enrichedQueue.length, 'patients');
      console.log('📊 [MyWaitingQueue] Détails des patients:', enrichedQueue);
          ...
          {appointments.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto max-h-96">
              {appointments.map((appointment) => {
                ...
                return (
                  <div key={appointment.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:bg-gray-100 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 bg-medical-primary rounded-lg flex items-center justify-center text-white font-bold text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Dossier: {appointment.patient?.numero_dossier}</p>
                      </div>
                    </div>
                    
                    <div className="space-y-2">
                      <p className="text-sm text-gray-500">{appointment.motif || 'Consultation'}</p>
